...
if ~sample.parallelFlag
    ...
    elseif strcmp(sample.algorithm,'ll_ACHRB') || strcmp(sample.algorithm,'HRB')
        
        % Perform parallel sampling using ll-ACHRB or HRB
        ...
        elseif strcmp(sample.algorithm,'HRB')   
            parfor workerIdx = 1:sample.numCores
                rng('shuffle');
                [points{workerIdx},samplingTime{workerIdx}] = HRB(sample,numPointsPerCore,sample.numDiscarded,sample.stepsPerPoint,1,workerIdx);
            end
        end
        delete(gcp('nocreate'));
        ...
        % Fill with the exact number of samples
        sample.points = sample.points(:,1:sample.numSamples);
        if strcmp(sample.algorithm,'HRB')
