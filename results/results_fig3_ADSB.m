...
save -v7.3 sample_ecoli_core_ADSB.mat sample
clearvars -except options

% Load iIT341
load('iIT341.mat');
sample = looplessFluxSampler(model,options);
save -v7.3 sample_iIT341_ADSB.mat sample
clearvars -except options

% Load iYO844
load('iYO844.mat');
sample = looplessFluxSampler(model,options);
save -v7.3 sample_iYO844_ADSB.mat sample
clearvars -except options

% Load iMM904
load('iMM904.mat');
sample = looplessFluxSampler(model,options);
save -v7.3 sample_iMM904_ADSB.mat sample
clearvars -except options